...
#######################################################################
This program is a modification of the code of the 
CH_08_maze program written by Shangtong Zhang
...
class DynaParams:
    def __init__(self):
        ...
        # algorithm names
        self.methods = ['Dyna-Q', 'Dyna-Q+ bonus update', 'Dyna-Q+ bonus action']

        # threshold for priority queue
        self.theta = 0
        
        # update bonuses in Dyna-Q+ case (TimeModel)
        self.update_bonus = True


# choose an action based on epsilon-greedy algorithm
def choose_action(state, q_value, maze, dyna_params, model, steps):
    if np.random.binomial(1, dyna_params.epsilon) == 1:
        return np.random.choice(maze.actions)
    else:
        values = q_value[state[0], state[1], :]
        if not dyna_params.update_bonus:
            if tuple(state) in model.model.keys():                    # if state was visited before
                values = values + [model.time_weight * (steps - model.model[tuple(state)][i][2]) ** 0.5 for i in list(range(4))]    
...
class TrivialModel:
    ...
    def feed(self, state, action, next_state, reward):
        ...
        if tuple(state) not in self.model.keys():
            self.model[tuple(state)] = dict()                                         # for each new state, a dictionary is created
        self.model[tuple(state)][action] = [list(next_state), reward]                 # for that state dictionary, the action becomes a key and next_state, reward the value
...
def dyna_q(q_value, model, maze, dyna_params):
    ...
    while state not in maze.GOAL_STATES:
        ...
        # get action
        action = choose_action(state, q_value, maze, dyna_params, model, steps)
        ...
        for t in range(0, dyna_params.planning_steps):
            state_, action_, next_state_, reward_ = model.sample(dyna_params.update_bonus)
...
def changing_maze(maze, dyna_params):
    ...
    for run in tqdm(range(dyna_params.runs)):
        # set up models
        models = [TrivialModel(),
                  TimeModel(maze, time_weight=dyna_params.time_weight),
                  TimeModel(maze, time_weight=dyna_params.time_weight)]
        ...
        for i in range(len(dyna_params.methods)):
            ...
            dyna_params.update_bonus = True
            if i == 2:
                dyna_params.update_bonus = False
...
def blocking_maze():
    ...
    plt.savefig('C:/Users/Hans/workspace/Reinforcement Learning/Files/images/Chapter 8/exercise_8_4_a.png')
    plt.close()
...
def short_cut_maze():
    ...
    plt.savefig('C:/Users/Hans/workspace/Reinforcement Learning/Files/images/Chapter 8/exercise_8_4_b.png')
    plt.close()
